...
二叉搜索树性质允许我们通过一个简单的递归算法来按序输出二叉搜索树中的所有关键字，这种算法称为中序遍历(inorder tree walk)算法。这样命名的原因是输出的子树根的关键字位于其左子树的关键字和右子树的关键字之间。类似地，先序遍历(preorder tree walk)中输出的根的关键字在其左右子树关键字之前，而后序遍历(postorder tree walk)输出的根的关键字在其左右子树的关键字之后。
...
通过从树根开始沿着left孩子指针直到遇到一个null,我们总能在一棵二叉搜索树中找到一给定节点x为根的最小节点。
...
二叉搜索树的性质保证了minimum是正确的，如果结点x没有左子树，那么由于x右子树中的每个关键字都至少大于或等于x.key，则以x为根的子树种的最小关键字是x.key。如果结点x有左子树，那么由于其右子树中没有关键字小于x.key，且在左子树中的每个关键字不大于x.key，则以x为根的子树的最小关键字一定在以x.left为根的子树中。
...
successor的代码分两种情况：如果结点x的右子树非空，那么x的后继恰好是x右子树中的最左结点，通过调用minimum可以得到。如果结点x的右子树非空并有一个后继y，那么y就是x的有左孩子的最底层祖先，并且它也是x的一个祖先。
