...
上图描述的是 Prim 算法的工作过程。算法首先将每个结点的 key 值设置为 ∞（除根结点 r 以外，根结点 r 的 key 值设为 0，以便使该结点成为第一个被处理的结点），将每个结点的父结点设置为 null，并对最小优先队列 Q 进行初始化，使其包含图中所有的结点。该算法维持的循环不变式由 3 个部分组成，在 while 循环开始前，我们有：
