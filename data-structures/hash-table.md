...
当然，理想的解决办法是避免所有的冲突。我们可以试图选择一个合适的散列函数h来做到这一点，一个想法就是使 h 尽可能的随机，从而避免冲突或使冲突的次数最小化。实际上，术语“散列”的原意就是随机混杂和拼凑，即体现了这种思想。当然，一个散列函数 h 必须是确定的，因为某一个给定的输入 k 应始终产生相同的结果 h(k)。但是，由于丨U丨> m，故至少有两个关键字其散列值相同，所以要完全避免冲突是不可能的。因此，我们一方面可以通过精心设计的散列函数来尽量减少冲突的次数，另一方面，仍需要有解决可能出现冲突的办法。
